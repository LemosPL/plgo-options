...
document.getElementById("btn-run-optv2").addEventListener("click", async () => {
  ...
  try {
    ...
    const data = await post("/api/optimization/run", {
      ...
      t90_weight: parseFloat(document.getElementById("optv2-t90-weight")?.value || "0.2"),
...
document.getElementById("btn-run-optv3")?.addEventListener("click", async () => {
  ...
  try {
    ...
    const data = await post("/api/optimization/run", {
      ...
      t90_weight: parseFloat(document.getElementById("optv3-t90-weight")?.value || "0.2"),
...
document.getElementById("btn-optv4-send-to-pricing")?.addEventListener("click", () => {
  ...
  for (const t of source) {
    ...
    let expCode = optv2ExpiryText(t);
    if (!/^\d{1,2}[A-Z]{3}\d{2}$/.test(expCode || "")) expCode = null;
    addLeg(side, opt, String(K), premium, String(qty), expCode, t.counterparty ? String(t.counterparty).toLowerCase() : "");
