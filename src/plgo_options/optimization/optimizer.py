...
class OptimizerV2(BaseOptimizer):
    ...
    def run(
        ...
        c_theta = np.array(
            [safe_num(c.theta) for c in candidates],
            dtype=float,
        )
        c_vega = np.array(
            [safe_num(c.vega) for c in candidates],
            dtype=float,
        )
        c_price = np.array(
            [max(safe_num(c.bs_price_usd), 0.0) for c in candidates],
            dtype=float,
        )
        c_iv_pct = np.array(
            [max(safe_num(c.iv_pct), 0.0) for c in candidates],
            dtype=float,
        )
        c_strike = np.array(
            [safe_num(c.strike) for c in candidates],
            dtype=float,
        )
        c_dte = np.array(
            [int(safe_num(c.dte)) for c in candidates],
            dtype=int,
        )
